...
export interface PremiumShot {
  id: string;
  sceneId?: string;
  kind: "hook" | "overview" | "action" | "result" | "transition";
  atMs: number;
  durationMs: number;
  title?: string;
  intent?: string;
...
export interface MotionOverlayCue {
  id: string;
  type: "caption" | "callout" | "spotlight" | "badge";
  /** Time offset in the raw recording timeline. The renderer maps this through speed ramps. */
  atMs: number;
  durationMs: number;
  text: string;
  subtext?: string;
  /** Target box in viewport-relative coordinates, for callouts/spotlights. */
  x?: number;
  y?: number;
  width?: number;
  height?: number;
...
export interface VisualPlanResult {
  ...
  /** Optional premium direction layer: shots, motion captions, callouts, and spotlights. */
  premium?: PremiumVisualPlan;
}
